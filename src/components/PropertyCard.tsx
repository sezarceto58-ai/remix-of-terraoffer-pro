    ...
    return (
      ...
      >
        <div className="relative aspect-[4/3] overflow-hidden">
          ...
          <button
            className={`absolute top-3 right-3 p-2 rounded-full backdrop-blur-sm transition-colors ${liked ? "bg-destructive/80 text-destructive-foreground" : "bg-background/40 text-foreground hover:text-primary"}`}
            onClick={(e) => {
              e.preventDefault();
              setLiked(!liked);
              toggleFav.mutate(property.id);
              toast({ title: liked ? "Removed from favorites" : "Added to favorites", description: property.title });
            }}
          >
          ...
          <div className="absolute bottom-3 left-3">
            ...
            {property.price_iqd && <p className="text-xs text-muted-foreground">IQD {property.price_iqd.toLocaleString()}</p>}
          </div>
        ...
        <div className="p-4">
          <h3 className="font-semibold text-foreground group-hover:text-primary transition-colors truncate">{property.title}</h3>
          <p className="flex items-center gap-1 mt-1 text-sm text-muted-foreground">
            <MapPin className="w-3 h-3" />
            {property.district}, {property.city}
          </p>
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border text-xs text-muted-foreground">
            {property.bedrooms > 0 && <span className="flex items-center gap-1"><Bed className="w-3.5 h-3.5" /> {property.bedrooms}</span>}
